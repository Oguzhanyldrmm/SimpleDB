...
public class RecoveryMgr {
   ...
   /**
    * Write a non-quiescent checkpoint record to the log and flush it.
    * @param txs the ids of the transactions active at checkpoint time
    */
   public void checkpoint(List<Integer> txs) {
      int lsn = NQCheckpoint.writeToLog(lm, txs);
      lm.flush(lsn);
      System.out.println("NQ CHECKPOINT: Transactions " + activeTxString(txs) + " are still active");
   }
   ...
   private void doRecover() {
      ...
      Collection<Integer> txsActiveAtCheckpoint = null;
      Iterator<byte[]> iter = lm.iterator();
      while (iter.hasNext()) {
         byte[] bytes = iter.next();
         LogRecord rec = LogRecord.createLogRecord(bytes);
         ...
         if (rec.op() == CHECKPOINT)
            return;
         if (rec.op() == NQCKPT) {
            txsActiveAtCheckpoint = ((NQCheckpoint) rec).txNumbers();
            txsActiveAtCheckpoint.removeAll(finishedTxs);
            if (txsActiveAtCheckpoint.isEmpty())
               return;
            continue;
         }
         if (rec.op() == COMMIT || rec.op() == ROLLBACK)
            finishedTxs.add(rec.txNumber());
         else if (txsActiveAtCheckpoint != null) {
            if (txsActiveAtCheckpoint.contains(rec.txNumber())) {
               if (rec.op() == START) {
                  txsActiveAtCheckpoint.remove(rec.txNumber());
                  if (txsActiveAtCheckpoint.isEmpty())
                     return;
               }
               else
   ...
   private String activeTxString(List<Integer> txs) {
      String str = "";
      for (Integer txnum : txs)
         str += txnum + " ";
      return str.trim();
   }
}
