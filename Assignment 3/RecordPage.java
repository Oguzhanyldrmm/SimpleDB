...
public class RecordPage {
   ...
   public boolean isNull(int slot, String fldname) {
      int flagval = tx.getInt(blk, offset(slot));
      return getBitVal(flagval, layout.bitPosition(fldname)) == 1;
   }

   public void setNull(int slot, String fldname) {
      setNullBit(slot, fldname, 1);
   }
   ...
   private void setFlag(int slot, int flag) {
      if (flag == EMPTY)
         tx.setInt(blk, offset(slot), EMPTY, true);
      else {
         int flagval = tx.getInt(blk, offset(slot));
         tx.setInt(blk, offset(slot), setBitVal(flagval, 0, USED), true);
      }
   }
   ...
   private int getFlag(int slot) {
      int flagval = tx.getInt(blk, offset(slot));
      return getBitVal(flagval, 0);
   }

   private void setNullBit(int slot, String fldname, int flag) {
      int flagpos = offset(slot);
      int flagval = tx.getInt(blk, flagpos);
      flagval = setBitVal(flagval, layout.bitPosition(fldname), flag);
      tx.setInt(blk, flagpos, flagval, true);
   }
